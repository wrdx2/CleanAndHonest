...
var Editer = new Class(
		{
			options : {
				...
				fontsizedata : {
					fontSize : [ "字号1", "字号2", "字号3", "字号4", "字号5", "字号6",
							"字号7" ],
					fontname : [ "宋体", "黑体", "楷体", "隶书", "幼圆", "Arial",
			...
			initialize : function(container, data, b, options) {
				...
				Css(this.textarea, {
					width : Sys.IE ? this.option.width - 2 + "px"
							: this.option.width + "px",
					height : Sys.IE ? this.option.height - 66 + "px"
							: this.option.height - 70 + "px"
				});
				Attr(this.iframe, {
					frameborder : 0,
					src : "about:blank",
					width : Sys.IE ? this.option.width : this.option.width - 4,
					height : Sys.IE ? this.option.height - 66
							: this.option.height - 70
				});
				Attr(this.original, {
					type : "checkbox",
					className : "checkbox",
				...
				this.bottom.insertBefore(this.original,
						this.bottom.childNodes[0]);
				this.ed = Sys.IE ? this.iframe.contentWindow.document
						: this.iframe.contentDocument;
				this.ed.open();
				var div = Sys.IE ? "<div></div>" : "";
				this.ed
						.write("<html><head><style>body{margin:5px;font-size:16px;word-wrap:break-word}</style></head><body id='my_body'>"
								+ div + "</body></html>");
				this.ed.close();
				...
				for ( var i = 0, l = data.length; i < l; i++) {
					...
					addListener(o, "mouseover", Bind(this, this.Changebgcolor,
							o, data[i].hover));
					addListener(o, "mouseout", Bind(this, this.Changebgcolor,
							o, data[i].Class));
					addListener(o, "click", Bind(this, this[data[i].action], o,
							data[i].args));
				}
				// ////////////////////////生成工具栏
				...
				addListener(this.original, 'click', Bind(this,
						this.Showoriginal, this.original)); // 显示源代码
			...
			Show : function() {
				...
				var elm = [
						[ spans[0], "Bold", "tag span0_active", "tag span0" ],
						[ spans[1], "Italic", "tag span1_active", "tag span1" ],
						[ spans[2], "Underline", "tag span2_active",
								"tag span2" ],
						[ spans[12], "InsertUnorderedList", "tag span3_active",
								"tag span12" ],
						[ spans[13], "InsertOrderedList", "tag span4_active",
								"tag span13" ] ];
			...
			InsertImage : function(o) {
				...
				this.lightbox.Show();
				this.makebody(this.Imagepopoup, "350px", "插入图片", "InsertImage",
						"Imagepopoup");
			},
			CreateLink : function(o) {
				if (Sys.IE)
					this.rang = this.ed.selection.createRange();
				this.lightbox.Show();
				this.makebody(this.Linkpopoup, "350px", "插入连接", "CreateLink",
						"Linkpopoup");
			},
			Fontcolor : function(o) {
				...
				if (!this.fontcolorpopup) {
					...
					var bgcolor = [ "00", "33", "66", "99", "CC", "FF", "00",
							"33", "66", "99", "CC", "FF" ];
					$$(color.container, "div")[1]
							.appendChild(createtab(
									12,
									18,
									function(tab) {
										Attr(tab, {
									...
									function(i, j, td) {
										...
										addListener(td, 'click', Bind(self,
												self.Execa, color, "fontColor"));
										addListener(td, 'mousedown',
			...
			Expression : function(o) {
				...
				if (!this.facebgpopup) {
					...
					$$(expr.container, "div")[1]
							.appendChild(createtab(
									1,
									this.option.facebg.length,
									function(tab) {
										Attr(tab, {
											cellPadding : 0,
											cellSpacing : 1,
			...
			Layout : function() {
				...
				for ( var i = 0, l = child.length; i < l; i++) {
					if (child[i].nodeName == "DIV" || child[i].nodeName == "P")
						child[i].style.textIndent = child[i].style.textIndent == "2em" ? ""
								: "2em";
				}
			},
			Fontsize : function(o) {
				if (!this.fontsizepopup) {
					...
					for ( var i = 0, l = this.option.fontsizedata.fontSize.length; i < l; i++) {
						...
						addListener(a, "click", Bind(this, this.Execa, i,
								"fontSize"));
						addListener(a, 'mousedown', BindAsEventListener(this,
								this.Bubble));
					}
				} else
			...
			FontName : function(o) {
				if (!this.fontnamepopup) {
					...
					for ( var i = 0, l = this.option.fontsizedata.fontname.length; i < l; i++) {
						...
						addListener(a, "click", Bind(this, this.Execa,
								this.option.fontsizedata.fontname[i],
								"fontname"));
						addListener(a, 'mousedown', BindAsEventListener(this,
								this.Bubble));
					}
			...
			makebody : function(o, w, t, f, n) {
				if (!o) {
					...
					this[n].elm = [ $$(this[n].container, "input")[0],
							$$(this[n].container, "img")[0],
							$$(this[n].container, "img")[1] ];
					this[n].elm[0].focus();
					addListener(this[n].elm[1], 'click', Bind(this, this.Execa,
							null, f));
					addListener(this[n].elm[2], 'click', function() {
						self.lightbox.Close();
						self[n].Close();
					});
				} else
			...
			Addtable : function() {
				...
				if (!this.tablepopup) {
					...
					this.tablepopup.elm = [ o[0], o[1], o[2], o[3],
							$$(this.tablepopup.container, "img")[0],
							$$(this.tablepopup.container, "img")[1] ];
					addListener(this.tablepopup.elm[4], 'click', Bind(this,
							this.Execa, null, "CreateTable"));
					addListener(this.tablepopup.elm[5], 'click', function() {
						self.lightbox.Close();
						self.tablepopup.Close();
					});
				} else
					with (this.tablepopup) {
						pos();
						Show();
						elm[0].focus();
					}
			...
			Execa : function(num, stamp) {
				var exec = {
					...
					CreateLink : function() {
						...
						if (this.Linkpopoup.elm[0].value == "")
							return;
						if (Sys.IE) {
							this.rang.execCommand("CreateLink", false,
									this.Linkpopoup.elm[0].value);
							this.rang.parentElement().target = "_blank ";
						} else {
							this.ed.execCommand("CreateLink", false,
									this.Linkpopoup.elm[0].value);
							this.rang = this.iframe.contentWindow
					...
					InsertImage : function() {
						...
						if (this.Imagepopoup.elm[0].value == "")
							return;
						Sys.IE ? this.rang.execCommand("InsertImage", false,
								this.Imagepopoup.elm[0].value) : this.ed
								.execCommand("InsertImage", false,
										this.Imagepopoup.elm[0].value);
					},
					Expression : function() {
					...
					CreateTable : function() {
						...
						if (Sys.IE) {
							this.rang.execCommand("InsertImage", false,
									"http://xxx.com/xxxxx.gif");
							p = this.rang.parentElement();
						} else {
							this.ed.execCommand("InsertImage", false,
									"http://xxx.com/xxxxx.gif");
							p = this.iframe.contentWindow.getSelection()
									.getRangeAt(0).commonAncestorContainer;
						}
						var tab = createtab(o[0].value, o[1].value, function(
								tab) {
							Attr(tab, {
								cellPadding : 0,
								cellSpacing : 1,
						...
						p.insertBefore(this.ed.createElement("br"),
								tab.nextSibling);
					}
				};
			...
			pos : function() {
				var str = "left:"
						+ (Math.max(document.documentElement.scrollWidth,
								document.documentElement.clientWidth) - this.w)
						/ 2
						+ "px;top:"
						+ ((Math.min(document.documentElement.scrollHeight,
								document.documentElement.clientHeight) - this.h) / 2 + document.documentElement.scrollTop)
						+ "px";
...
var LightBox = {
	...
	init : function(options) {
		...
		delete this.config;
		this.obj = document.body.insertBefore(document.createElement("div"),
				document.body.childNodes[0]);
		var str = "display:none; z-index:"
				+ this.zIndex
				+ ";left:0px;top:0px;position:fixed;width:100%;height:100%;background-color:"
				+ this.Color
				+ (Sys.IE ? ";filter : alpha(opacity:" + this.Opacity + ")"
		...
		if (Sys.IE6) {
			...
			this._resize = function() {
				_self.obj.style.width = Math.max(
						document.documentElement.scrollWidth,
						document.documentElement.clientWidth)
						+ "px";
				_self.obj.style.height = Math.max(
